...
export default function TagsPaginate({ page, limit, count, link }) {
  //   console.log(count);
  page = page ? page : 1;
  return (
    <div className="px-4 flex justify-between py-3">
      ...
      >
        <Link href={`${link}&page=${page ? parseInt(page) - 1 : 2}`}>
          <a className="tracking-widest font-medium text-white">Previous</a>
        </Link>
      </div>
      ...
      >
        <Link href={`${link}&page=${page ? parseInt(page) + 1 : 2}`}>
          <a className="tracking-widest font-medium text-white">Next</a>
        </Link>
      </div>
    </div>
  );
