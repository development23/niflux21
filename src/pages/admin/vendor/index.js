...
export async function getServerSideProps({ query }) {
  ...
  const limit = 10;
  await dbConnect();
  const vendorsCount = await VendorModel.count();
...
export default function Vendor({ vendors, vendorsCount, limit }) {
  const color = "dark";
                ...
                {vendors.map((vendor, index) => (
                  <tr key={vendor._id}>
                      ...
                      <div className="flex items-center">
                        <i
                          className={`fas fa-circle ${
                            vendor.active ? "text-green-500" : "text-red-500"
                          } mr-2`}
                        ></i>
                    ...
                    <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-left">
                      {moment(vendor.createdAt).format("Do, MMM YY hh:mm a")}
                    </td>

                    <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-left">
                      <Link
                        href={{
                          pathname: "/admin/vendor/[vendor]",
