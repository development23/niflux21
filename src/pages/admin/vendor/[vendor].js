...
export async function getServerSideProps({ query }) {
  const { vendor, page } = query;
  // console.log(vendor);
  const limit = 10;

  await dbConnect();
  const vendorData = await VendorModel.findOne({ _id: vendor });

  const propertyCount = await Property.find({ vid: vendor }).count();
  const properties = await Property.find({ vid: vendor })
    .limit(limit)
    .sort({ createdAt: "desc" })
...
export default function Vendor({ vendor, propertyCount, properties, limit }) {
  const color = "dark";
  const router = useRouter();

  const handleVendorDeletion = (vendorId) => {
    const r = confirm(
    ...
    axios
      .delete(`/api/admin/vendor?vendor=${vendorId}`)
      .then(({ data }) => {
        console.log(data);
        router.replace("/admin/vendor");
      })
      ...
      <div className="px-2 py-3 bg-slate-600 rounded pl-4 text-white shadow mb-5 backdrop-blur-[5px] space-y-1">
        <div className="justify-between flex">
          <ul className="flex justify-start mt-1">
            ...
            <li className="pr-2 text-[16px]  text-[#ffffff]">
              <a href="/admin/vendor"> {vendor.name} </a>
            </li>
          </ul>
          ...
          <div className="rounded-t mb-0 px-4 py-3 border-0">
            <div className="flex flex-wrap items-center">
              <div className="relative w-full px-4 max-w-full text-center flex-grow flex-1">
                ...
                >
                  <span className="capitalize">{vendor.name} Details</span>
                </h2>
              </div>

              <IconButton
                aria-label="delete"
                onClick={() => router.push("edit-vendor?key=" + vendor._id)}
              >
              ...
              <IconButton
                aria-label="delete"
                onClick={() => handleVendorDeletion(vendor._id)}
              >
              ...
              <div className="flex flex-wrap overflow-hidden xl:-mx-4">
                <div className="w-full overflow-hidden xl:my-2 xl:px-4 xl:w-full">
                  <p className="text-[18px] ">
                    Name :{" "}
                    <span className="text-[18px] "> {vendor?.name} </span>
                  </p>
                </div>
                ...
                <div className="w-full overflow-hidden xl:my-2 xl:px-4 xl:w-1/2">
                  <p className="text-[17px] ">
                    Mobile No :{" "}
                    <span className="text-[16px] "> +91-{vendor?.phone} </span>
                  </p>
                </div>
                ...
                <div className="w-full overflow-hidden xl:my-2 xl:px-4 xl:w-1/2">
                  <p className="text-[17px] ">
                    Email :{" "}
                    <span className="text-[16px] "> {vendor?.email} </span>
                  </p>
                </div>
                ...
                <div className="w-full overflow-hidden xl:my-2 xl:px-4 xl:w-1/2">
                  <p className="text-[17px] ">
                    City :{" "}
                    <span className="text-[16px] "> {vendor?.city} </span>
                  </p>
                </div>
                ...
                <div className="w-full overflow-hidden xl:my-2 xl:px-4 xl:w-1/2">
                  <p className="text-[17px] ">
                    State :{" "}
                    <span className="text-[16px] "> {vendor?.state} </span>
                  </p>
                </div>
                ...
                <div className="w-full overflow-hidden xl:my-2 xl:px-4 xl:w-full">
                  <p className="text-[17px] ">
                    Address :{" "}
                    <span className="text-[16px] "> {vendor?.address} </span>
                  </p>
                </div>
