...
export async function getServerSideProps(ctx) {
  const vendorSession = await getSession(ctx);
  const { page } = ctx.query;
  const limit = 10;
  await dbConnect();
  const propertiesCount = await Property.count();
  const properties = await Property.find({ vid: vendorSession.id })
    .limit(limit)
    .sort({ createdAt: "desc" })
...
export default function PropertyManagement({
  ...
  return (
    <>
      <div className="px-2 py-3 bg-slate-600 rounded pl-4 text-white shadow mb-5 backdrop-blur-[5px] space-y-1 flex justify-between items-center">
        <ul class="flex justify-start ">
          <li className="pr-2 text-[16px] text-[#ffffff]">
            <Link href="/admin/dashboard"> Home </Link>{" "}
          </li>
          <li className="pr-2">
            <i class="fas fa-chevron-right text-[14px] text-[#ffffff]"></i>
          </li>
        ...
        <ul className="flex justify-end ">
          <li className="pr-2 text-[16px] bg-[#fefefe] text-[#313131] font-semibold pl-4  pr-5 py-2 rounded-3xl ">
            <Link href="property/add-property"> Add New Property </Link>
          </li>
        </ul>
      </div>
                  ...
                  <tr key={item._id}>
                    <th className="border-t-0 pr-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-left capitalize">
                      ...
                      >
                        <Link
                          href={{
                            pathname: "/vendor/property/[property]",
